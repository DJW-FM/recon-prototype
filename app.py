...
show_subtotals = st.sidebar.checkbox("Vis subtotaler", value=True, key="show_subtotals_toggle")
view = st.sidebar.selectbox("Visning", ["Kontoplan-rækkefølge", "Rapporteringskategori"], key="view_mode")
filters = st.sidebar.multiselect("Filter", ["Kun ikke afstemt", "Kun ikke reviewet"], key="filters_multiselect")
sort_radio_value = st.sidebar.radio("Sortering i gruppe", ["Numerisk kontonr.", "Filens rækkefølge"], index=0, key="sort_choice")

# Apply sort choice to internal key
if sort_radio_value == "Numerisk kontonr.":
